...
final class WebHook
{
     ...
     */
    public static function generate(string $path, string $namespace, string $baseNamespace, string $className, PathItem $pathItem, SchemaRegistry $schemaRegistry, string $rootNamespace): iterable
    {
                    ...
                    foreach ($content->schema->oneOf as $oneOfSchema) {
                        foreach ($oneOfSchema->properties as $name => $property) {
                            if ($content->schema->discriminator->propertyName === $name) {
                                ...
                                $fabicatedSchema = new \cebe\openapi\spec\Schema([
