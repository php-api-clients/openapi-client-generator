...
/**
 * @covers \ApiClients\Client\PetStore\Internal\Operation\Pets\List_
 */
final class List_Test extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    ...
    public function call_httpCode_default_responseContentType_application_json_zero()
    {
        ...
        $browser->request('GET', '/pets?page=1&per_page=8', \Prophecy\Argument::type('array'), \Prophecy\Argument::any())->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\PetStore\Client($auth->reveal(), $browser->reveal());
        $result = $client->call(Internal\Operation\Pets\List_::OPERATION_MATCH, (static function (array $data) : array {
            $data['per_page'] = 8;
            $data['page'] = 1;
            return $data;
        })(array()));
    ...
    public function operations_httpCode_default_responseContentType_application_json_zero()
    {
        ...
        $browser->request('GET', '/pets?page=1&per_page=8', \Prophecy\Argument::type('array'), \Prophecy\Argument::any())->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\PetStore\Client($auth->reveal(), $browser->reveal());
        $result = $client->operations()->pets()->list(8, 1);
    }
}
